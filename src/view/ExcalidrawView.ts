...
import { diagramToHTML, diagramToHTMLByDify } from "../utils/matic";
      ...
      this.packages.excalidrawLib.DiagramToCodePlugin,
      {
        generate: async ({ frame, children }: 
          ...
          try {
            ...
            const response = await diagramToHTMLByDify ({
              image:dataURL,
            ...
            console.log("generatedResponse HTML  ", generatedResponseRaw)

              // 截取 ```html  和 ```之间的内容
              const html = generatedResponseRaw
              .split("```html")[1]
              ?.split("```")[0]
              ?.trim();
